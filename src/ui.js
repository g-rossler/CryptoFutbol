...
import {
  comprarJugador,
  entregarPremio,
} from "../back/transaccionesStellar";
...
function crearJugadoresTitulares(baseDatos) {
  ...
  for (let i = 0; i <= 10; i++) {
    ...
    $atq.innerText = baseDatos.jugadores.titulares[i].habilidades.ataque;
    const $def = document.createElement("td");
    $def.innerText = baseDatos.jugadores.titulares[i].habilidades.defensa;
    const $res = document.createElement("td");
    $res.innerText = baseDatos.jugadores.titulares[i].habilidades.resistencia;
...
function crearJugadoresSuplentes(baseDatos) {
  ...
  for (let i = 0; i <= 4; i++) {
    ...
    const $atq = document.createElement("td");
    $atq.innerText = baseDatos.jugadores.suplentes[i].habilidades.ataque;
    const $def = document.createElement("td");
    $def.innerText = baseDatos.jugadores.suplentes[i].habilidades.defensa;
    const $res = document.createElement("td");
    $res.innerText = baseDatos.jugadores.suplentes[i].habilidades.resistencia;
...
function verificarSaldo() {
  ...
  document.querySelector("#ftok-saldo").innerText = `${datosUsuario} $FTOK`;
}

function crearBotones() {
  document.querySelector("#siguiente-fecha").onclick = () => entregarPremio();
